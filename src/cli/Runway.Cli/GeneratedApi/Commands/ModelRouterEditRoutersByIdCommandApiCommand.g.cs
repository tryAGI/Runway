...
internal static partial class ModelRouterEditRoutersByIdCommandApiCommand
{
    ...
    private static Option<string?> NameOption { get; } = new(
        name: @"--name")
    {
        Description = @"Display name. The slug is immutable and cannot be changed after creation.",
    };
    ...
    private static Option<global::Runway.PatchRoutersRequestSettings?> Settings { get; } = new(
        name: @"--settings")
    {
        Description = @"Nested merge: omitted settings fields keep their current values. When models is present, omitted models.mode or models.ids are preserved (sending only optimizeFor does not clear the model allowlist or credit ceiling).",
    };
      ...
      private static Option<string?> RequestFile { get; } = new(@"--request-file")
      {
          ...
          Hidden = true,
                    ...
                    private static string FormatResponse(ParseResult parseResult, global::Runway.PatchRoutersResponse value, global::System.Text.Json.Serialization.JsonSerializerContext context, bool truncateLongStrings)
                    {
                        string? text = null;
                    ...
                    static partial void CustomizeResponseText(ParseResult parseResult, global::Runway.PatchRoutersResponse value, ref string? text);
...
Update a Model Router configuration. Settings changes append a new version; name and description updates do not. Settings are merged with the current snapshot — omitted fields keep their existing values.");
        ...
        command.SetAction(async (ParseResult parseResult, CancellationToken cancellationToken) =>
            ...
            {
                        var __requestBase = await CliRuntime.ReadRequestOrDefaultAsync<global::Runway.PatchRoutersRequest>(
                            parseResult,
                            Input,
                            RequestJson,
                            RequestFile,
                ...
                using var client = await CliRuntime.CreateClientAsync(parseResult, cancellationToken).ConfigureAwait(false);
                                ...
                                var response = await client.ModelRouter.EditRoutersByIdAsync(
                                    id: id,
